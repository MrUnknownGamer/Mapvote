...
---------------------------------------- ]]
if SERVER then
MapLockThreshold = CreateConVar( "glmvs_maplockthreshold", 0.4, FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Percentage number of maps that required to be played until list refresh." ):GetFloat()
...
MVotePower = CreateConVar( "glmvs_mvotepower", -1, FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Maximum vote power for the players. Set -1 or 0 to disable." ):GetInt()
...
VoteDelay = CreateConVar( "glmvs_votedelay", 1, FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Vote delay between votes made by the player, to prevent spam (In seconds)." ):GetInt()
...
RTVMode = CreateConVar( "glmvs_enablertv", 1, FCVAR_REPLICATED + FCVAR_ARCHIVE + FCVAR_NOTIFY, "Enables the RTV feature if you want it for the server." ):GetBool()
